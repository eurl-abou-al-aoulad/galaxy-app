...
const lanSync = require("./lanSync.cjs");
...
// ===== IPC: LAN Sync hub control =====
ipcMain.handle("galaxy:lan-start", async (_event, opts) => {
  try { return { ok: true, status: lanSync.start(opts || {}) }; }
  catch (e) { return { ok: false, error: e?.message || String(e) }; }
});
ipcMain.handle("galaxy:lan-stop", async () => {
  try { return { ok: true, status: lanSync.stop() }; }
  catch (e) { return { ok: false, error: e?.message || String(e) }; }
});
ipcMain.handle("galaxy:lan-status", async () => {
  try { return { ok: true, status: lanSync.getStatus() }; }
  catch (e) { return { ok: false, error: e?.message || String(e) }; }
});
